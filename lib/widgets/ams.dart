...
class AmsSelection extends StatefulWidget {
  const AmsSelection({super.key, required this.ams, required this.printer});
...
class _AmsSelectionState extends State<AmsSelection> {
  ...
  Widget build(BuildContext context) {
    final String printerid = widget.printer.id.toString();
    final Ams ams = widget.ams;
                                          ...
                                          if (spool != null) ...[
                                            FilamentCard(
                                              ...
                                              deleteCallback: () async {
                                                await availableFilaments.unassignSpool(printerid, ams.id.toString(), ams.isExternalSpool ? "0" : tray.id.toString());
                                                if (!context.mounted) {
                                                  return;
                                                }
                                          ...
                                          if (widget.printer.status != null && widget.printer.status!.connected)
                                            InfoCard(
                                              ...
                                              onTap: () async {
                                                ...
                                                final res = await availablePrinters.resetSlot(printerid, ams.id.toString(), ams.isExternalSpool ? "0" : tray.id.toString());
                                                if (!context.mounted) {
                                                  return;
                                                }
                                                if (res == true) {
                                                  SnackbarService.success("Slot reset successfully!");
                                                } else {
                                                  SnackbarService.error("Slot reset failed!");
                                                }
                                                Navigator.pop(context);
                                              },
                                            ),
